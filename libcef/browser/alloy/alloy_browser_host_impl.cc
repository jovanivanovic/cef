...
class CefMediaAccessCallbackImpl : public CefMediaAccessCallback {
 public:
  ...
  ~CefMediaAccessCallbackImpl() { if (!callback_.is_null()) { Cancel(); } }
  ...
  void Continue(int32_t allowed_permissions) override {
    if (CEF_CURRENTLY_ON_UIT()) {
      ...
      CEF_POST_TASK(CEF_UIT, base::Bind(&CefMediaAccessCallbackImpl::RunNow,
    ...
    if (client.get()) {
      ...
      if (handler.get()) {
        ...
        CefRefPtr<CefFrame> frame =
            browser->GetFrame(request.render_process_id, request.render_frame_id);
